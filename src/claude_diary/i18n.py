"""Internationalization labels (Korean / English)."""

LABELS = {
    "ko": {
        "title": "작업일지",
        "auto_generated": "이 파일은 Claude Code Stop Hook에 의해 자동 생성됩니다.",
        "auto_appended": "각 세션이 종료될 때마다 작업 내용이 자동으로 기록됩니다.",
        "task_requests": "작업 요청",
        "files_created": "생성된 파일",
        "files_modified": "수정된 파일",
        "commands": "주요 명령어",
        "summary": "작업 요약",
        "brief_summary": "요약",
        "results": "결과",
        "report_summary": "요약",
        "work_report": "진행 업무",
        "work_state": "상태",
        "issues_risks": "이슈 / 리스크",
        "next_actions_support": "다음 액션 / 지원 요청",
        "command_file_commit_evidence": "명령 / 파일 / 커밋 근거",
        "artifacts": "증적",
        "work_overview": "작업 개요",
        "work_details": "작업 상세",
        "work_snapshot": "작업 한눈에",
        "item": "항목",
        "content": "내용",
        "context": "배경",
        "scope": "범위",
        "approach": "접근",
        "outcome": "결과",
        "impact": "영향",
        "work_highlights": "작업 하이라이트",
        "key_changes": "주요 변경사항",
        "code_change_highlights": "주요 코드 변경",
        "decisions": "결정 / 트레이드오프",
        "implementation_notes": "구현 메모",
        "decisions_and_notes": "결정 및 구현 메모",
        "verification": "검증",
        "validation": "검증",
        "validation_status": "검증 및 상태",
        "verification_and_followups": "검증 및 후속 조치",
        "risks": "주의사항 / 리스크",
        "next_steps": "다음 단계",
        "next_actions": "다음 액션",
        "support_needed": "필요한 지원 / 결정",
        "risks_next_actions": "리스크 / 다음 액션",
        "risks_next_steps": "리스크 및 다음 단계",
        "evidence": "원자료",
        "appendix": "부록",
        "prompt_outputs": "프롬프트 결과물 / 검증 산출물",
        "developer_evidence": "개발 근거",
        "raw_evidence": "원문 요청",
        "issues": "발생한 이슈",
        "session_id": "세션 ID",
        "categories": "카테고리",
        "git": "Git",
        "branch": "브랜치",
        "commit": "커밋",
        "code_stats": "변경 통계",
        "secrets_masked": "시크릿 마스킹됨",
        "weekdays": ["월", "화", "수", "목", "금", "토", "일"],
        "weekday_suffix": "요일",
        # CLI messages
        "cli_no_results": "'%s'에 대한 결과가 없습니다",
        "cli_found_entries": "%d개 항목을 찾았습니다:",
        "cli_no_index": "인덱스가 없습니다. 인덱스를 생성합니다...",
        "cli_no_match": "일치하는 항목이 없습니다.",
        "cli_no_history": "'%s'에 대한 이력이 없습니다",
        "cli_file_trace": "'%s' 파일 추적 (%d개 항목):",
        "cli_rebuilding": "검색 인덱스 재구축 중...",
        "cli_indexed": "%d개 세션 인덱싱 완료.",
        "cli_delete_confirm": "%s의 마지막 세션 엔트리를 삭제할까요? [y/N]: ",
        "cli_cancelled": "취소됨.",
        "cli_deleted": "%s에서 마지막 세션 엔트리가 삭제되었습니다",
        "cli_session_not_found": "세션 '%s'을(를) 찾을 수 없습니다.",
        "cli_audit_entries": "감사 로그 (%d개 항목):",
        "cli_audit_empty": "감사 로그가 없습니다.",
        "cli_checksum_ok": "체크섬 정상: %s",
        "cli_checksum_fail": "경고: 체크섬 불일치!",
        "cli_init_done": "완료! Claude Code 세션이 자동으로 기록됩니다.",
        "cli_migrating": "v1.0 환경변수를 config.json으로 마이그레이션 중...",
        "cli_team_not_configured": "팀이 설정되지 않았습니다. 실행: claude-diary team init --repo <url>",
        "cli_dashboard_generated": "대시보드 생성: %s",
    },
    "en": {
        "title": "Work Diary",
        "auto_generated": "This file is auto-generated by Claude Code Stop Hook.",
        "auto_appended": "Work content is automatically recorded at the end of each session.",
        "task_requests": "Task Requests",
        "files_created": "Files Created",
        "files_modified": "Files Modified",
        "commands": "Key Commands",
        "summary": "Work Summary",
        "brief_summary": "Summary",
        "results": "Results",
        "report_summary": "Summary",
        "work_report": "Work",
        "work_state": "State",
        "issues_risks": "Issues / Risks",
        "next_actions_support": "Next Actions / Support",
        "command_file_commit_evidence": "Command / File / Commit Evidence",
        "artifacts": "Artifacts",
        "work_overview": "Work Overview",
        "work_details": "Work Details",
        "work_snapshot": "Work Snapshot",
        "item": "Item",
        "content": "Content",
        "context": "Context",
        "scope": "Scope",
        "approach": "Approach",
        "outcome": "Outcome",
        "impact": "Impact",
        "work_highlights": "Work Highlights",
        "key_changes": "Key Changes",
        "code_change_highlights": "Code Change Highlights",
        "decisions": "Decisions / Trade-offs",
        "implementation_notes": "Implementation Notes",
        "decisions_and_notes": "Decisions and Implementation Notes",
        "verification": "Verification",
        "validation": "Validation",
        "validation_status": "Validation and Status",
        "verification_and_followups": "Verification and Follow-ups",
        "risks": "Notes / Risks",
        "next_steps": "Next Steps",
        "next_actions": "Next Actions",
        "support_needed": "Support / Decision Needed",
        "risks_next_actions": "Risks / Next Actions",
        "risks_next_steps": "Risks and Next Steps",
        "evidence": "Evidence",
        "appendix": "Appendix",
        "prompt_outputs": "Prompt Outputs / Verification Artifacts",
        "developer_evidence": "Developer Evidence",
        "raw_evidence": "Original Requests",
        "issues": "Issues Encountered",
        "session_id": "Session ID",
        "categories": "Categories",
        "git": "Git",
        "branch": "Branch",
        "commit": "Commit",
        "code_stats": "Code Stats",
        "secrets_masked": "secrets masked",
        "weekdays": ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
        "weekday_suffix": "",
        # CLI messages
        "cli_no_results": "No results found for '%s'",
        "cli_found_entries": "Found %d entries:",
        "cli_no_index": "No index found. Building index...",
        "cli_no_match": "No entries match the filter.",
        "cli_no_history": "No history found for '%s'",
        "cli_file_trace": "File trace for '%s' (%d entries):",
        "cli_rebuilding": "Rebuilding search index...",
        "cli_indexed": "Indexed %d sessions.",
        "cli_delete_confirm": "Delete last session entry from %s? [y/N]: ",
        "cli_cancelled": "Cancelled.",
        "cli_deleted": "Last session entry deleted from %s",
        "cli_session_not_found": "Session '%s' not found.",
        "cli_audit_entries": "Audit log (%d entries):",
        "cli_audit_empty": "No audit log entries found.",
        "cli_checksum_ok": "Checksum OK: %s",
        "cli_checksum_fail": "WARNING: Checksum mismatch!",
        "cli_init_done": "Done! Claude Code sessions will be auto-logged.",
        "cli_migrating": "Migrating v1.0 environment variables to config.json...",
        "cli_team_not_configured": "Team not configured. Run: claude-diary team init --repo <url>",
        "cli_dashboard_generated": "Dashboard generated: %s",
    },
}


def get_label(key, lang="ko"):
    """Get localized label."""
    lang = lang if lang in LABELS else "ko"
    return LABELS[lang].get(key, key)
