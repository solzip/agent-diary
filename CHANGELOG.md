...
> **한 줄 요약**: README를 코드와 문장 단위로 전수 대조한 감사가 만든 릴리스입니다.
> 감사가 찾은 **코드 결함 2건**을 고쳤고, 문서가 코드와 다르게 답하던 곳들을 맞췄습니다.
>
> - **`team monthly --month`가 월을 무시하고 현재 주의 주간 리포트를 내던 문제** —
>   이제 진짜 월간 리포트를 만듭니다
> - **브랜치 차수 `(#N)`이 세션의 2번째 턴부터 1 크게 찍히던 문제** — 4.12.0이 고친
>   자리의 잔여 off-by-one이 호출부에 있었습니다
> - PyPI 페이지에서 README의 상대 링크 12개와 데모 이미지가 깨져 있던 것도 이
>   릴리스의 sdist부터 고쳐집니다 (PyPI는 상대경로를 재작성하지 않습니다)
...
> **한 줄 요약**: 항목의 브랜치 줄에 붙는 차수 `(#N)`이 **세션이 아니라 턴을
> 세고 있었습니다.** 실측 인덱스에서 한 브랜치가 `(#1293)`으로 찍히려던 것이
> 실제로는 **25번째 세션**입니다.
>
> - 4.9.0에서 항목이 턴 단위가 되면서 갈라진 값입니다. 그 사이에 쓰인 항목의
>   차수는 **턴 수로 읽어야 합니다** — 이미 기록된 항목은 소급해 고치지 않습니다
> - Notion 스키마에서 **아무것도 채우지 않던 속성 2개**를 뺐습니다. `ensure`가
>   더 이상 만들지 않습니다. 이미 있는 DB의 컬럼과 값은 건드리지 않습니다
> - 두 변경 모두 밖에서 관찰됩니다. patch가 아니라 minor인 이유입니다
...
- **브랜치 차수 `(#N)`이 세션이 아니라 항목을 세고 있던 문제**: `count_branch_sessions`가 인덱스 **행 수**를 셌습니다. 쓰일 당시엔 그게 세션 수와 같았지만, **4.9.0에서 항목이 턴 단위가 되면서 달라졌습니다** — 그 뒤로 이 숫자는 턴 수입니다
  ...
  - **실측**(실제 `.diary_index.json`, 항목 7,269개): `project-a`의 한 브랜치가 **항목 1,292개 / 세션 24개**. 4.9.0 이후 항목만 봐도 `project-c`의 브랜치가 **항목 15개 / 세션 1개** — 다음 항목이 `(#16)`으로 찍히지만 실제로는 첫 세션입니다
...
> **한 줄 요약**: 항목이 카테고리를 여러 개 달아도 **첫 번째만** 검색 인덱스와
> 통계에 들어가고 있었습니다. 실측 일지 기준 **항목의 96%가 영향**을 받습니다.
>
> **올린 뒤 `agent-diary reindex`를 한 번 돌리세요.** 원본이 일지 파일에 그대로
> 있어서 **이번에는 완전히 복구됩니다** — 4.9.0 때와 달리 잃어버린 게 없습니다.
>
> - `agent-diary search docs`처럼 카테고리로 찾던 검색이 그동안 결과의 일부만
>   돌려주고 있었습니다. `refactor`는 실제 1,183건 중 **35건**만 답했습니다
> - `stats`의 카테고리 집계도 같은 이유로 틀렸습니다. 실측 20,424개 중 7,048개만
>   세고 있었고, **없는 카테고리를 만들어내기도** 했습니다
> - 일지 파일 자체는 처음부터 정확했습니다. 틀린 건 그것을 읽는 쪽입니다

### Fixed

- **카테고리가 항목마다 첫 개만 기록되던 문제**: `indexer.py`와 `lib/stats.py`가 **같은 정규식을 각자 한 벌씩** 들고 있었습니다
  ```python
  re.findall(r'(?:카테고리|Categories).*?`([^`]+)`', session)
  ```
  전부 모으는 것처럼 보이지만 정확히 하나만 모읍니다. 라벨은 항목당 한 번 나오므로 `findall`이 한 번 매치하고, 게으른 `.*?`가 첫 백틱 쌍에서 멈춥니다
  - **실측(실제 일지 73개 파일, 항목 7,131개)**: 카테고리 3개인 항목이 **91.6%**, 2개가 4.5% — 즉 **96.1%(6,853개)가 두 번째 이후를 잃고 있었습니다.** 파일에 20,424개가 있는데 `stats`는 7,048개를 셌습니다. `refactor` 35 vs 1,183, `docs` 2,849 vs 5,431
  - **없는 카테고리도 만들어냈습니다.** 산문에 "카테고리"라는 단어가 있으면(커밋 메시지, FAQ 인용) 그 옆 백틱을 집어서 `reindex`·`search`·`([^` 같은 값이 통계에 들어갔습니다. 실제 일지에서 그런 줄이 166개 나왔습니다
  - 라벨 **줄에 고정**하는 패턴으로 바꿨습니다. 실제 일지의 카테고리 줄 7,040개가 전부 볼드 형식(`**🏷️ 카테고리:**`)이고, 비볼드로 잡히던 166개는 전부 산문이었습니다. 수정 후 **20,424개, 잡값 0개** — 파일과 정확히 일치합니다
  - 패턴을 **한 곳에 둡니다**(`lib/stats.py`의 `CATEGORY_LINE`). 두 모듈이 각자 복사본을 들고 있었기 때문에 인덱스와 통계가 **같은 틀린 답에 사이좋게 합의**하고 있었습니다
  - **기존 테스트가 존재한 적 없는 형식을 고정하고 있었습니다.** 픽스처가 `Categories: \`frontend\``였는데 writer는 그 형식을 만들지 않습니다. 실제 형식으로 바꿨습니다
  - `test_reindex_fidelity.py`의 픽스처에는 **처음부터 카테고리가 두 개 들어 있었는데 아무 테스트도 단정하지 않았습니다.** 그래서 정규식을 되돌려도 28개가 전부 통과했습니다. 두 자리 모두 변형으로 검증했습니다 — **2/2**


## [4.11.2] - 2026-08-14

> **한 줄 요약**: 4.11.1이 죽는 건 막았지만 **차트가 `????????`로 나왔습니다.**
> 이제 못 그리는 문자 자리에 가장 가까운 ASCII를 그립니다 — `█`→`#`,
> `╔═╗`→`+=+`, `—`→`-`.
>
> - cp949 콘솔에서 `stats`·`weekly`·`report` 출력의 **`?`가 0개**가 됩니다
> - **저장되는 파일은 그대로입니다.** `weekly`는 리포트를 UTF-8로 파일에 쓴 뒤
>   인쇄하는데, 바뀌는 건 인쇄되는 사본뿐입니다. 일지 항목 헤더(`### ⏰ ... 📁`)도
>   손대지 않습니다 — 이 저장소의 파서가 그걸 보고 읽습니다
> - UTF-8 콘솔은 전과 동일합니다

### Fixed

- **인코딩할 수 없는 문자가 `?`로 바뀌어 차트를 못 읽던 문제**: 4.11.1은 `errors="replace"`로 예외만 막았습니다. 그런데 `stats`는 차트를 **블록 문자로 그립니다.** 막대 하나가 통째로 `????????`가 되면 그건 차트가 아니라 **고장난 프로그램으로 읽힙니다**
  - 스트림에 **커스텀 인코딩 에러 핸들러**를 등록합니다. 호출부를 감싸지 않은 이유: `stats`에만 `print`가 29개이고, 일지 내용은 대여섯 개 명령에서 터미널로 나오며 전부 `⏰`·`📁`를 달고 있습니다. **핸들러 하나가 그 전부를 덮고, 앞으로 쓰일 것까지 덮습니다**
  - 치환은 **길이를 보존합니다.** `stats`는 상자 너비를 글자 수로 계산하므로 길이가 달라지면 오른쪽 테두리가 밀립니다. 이모지는 터미널에서 두 칸을 먹으므로 한 글자 ASCII가 오히려 정렬이 낫습니다
  - **표에 없는 문자는 종류에 따라 갈립니다.** 처음엔 전부 `*`로 했는데, ASCII 로케일에서 돌려보니 `주간 작업 리포트`가 **`** ** ***`**가 됐습니다. 사라진 게 제목인데 장식처럼 보입니다. 글자·숫자는 `?`(내용이 사라졌다는 관용 표기), 기호·문장부호는 `*`(장식)로 나눴습니다
  - **`·`를 놓칠 뻔했습니다.** 세션 없는 날 표시인데 **cp949에는 있어서** 첫 검사에서 안 걸렸습니다. ASCII·cp932에는 없어서 한 달이 `*` 31개로 나왔습니다. 한 인코딩만 보고 끝내면 안 된다는 뜻이라, cp1252·ASCII·cp932까지 돌려봤습니다
  - **파일에는 적용되지 않습니다.** 터미널의 한계이지 파일의 한계가 아닙니다. `weekly`가 저장한 리포트에 `📊`와 `—`가 남아 있는 것, 일지 파일의 `⏰`·`📁`가 그대로인 것을 테스트로 고정했습니다
  - 실측: 수정 전 `weekly` 출력 546자 중 `?` 9개, `stats`의 막대는 전부 `?`. **수정 후 세 명령 모두 `?` 0개**


## [4.11.1] - 2026-08-14

> **한 줄 요약**: 한글 Windows 콘솔(cp949)에서 **명령 4개가 아예 못 돌고
> 있었습니다.** 출력에 `—`나 이모지가 하나라도 있으면 `print`가 예외를 던지고
> 명령이 통째로 죽습니다. 다른 프로젝트에서 `diary-notion push --dry-run`을
> 쓰다 제보받았습니다.
>
> - `stats`는 **첫 글자 `╔`에서**, `weekly`는 `📊`에서, `report`와 **`doctor`**도
>   같은 이유로 죽었습니다. 실행 가능한 명령 11개 중 4개입니다
> - `PYTHONIOENCODING=utf-8`로 우회하던 분들은 이제 안 해도 됩니다
> - **cp949 콘솔에서 한글은 그대로 나옵니다.** 인코딩할 수 없는 문자만 `?`가
>   됩니다. UTF-8 콘솔은 아무것도 잃지 않습니다

### Fixed

- **한글 Windows 콘솔에서 출력 도중 명령이 죽던 문제**: cp949는 한글과 `→`는 담지만 `—`(U+2014, 대신 U+2015를 가지고 있습니다)·`✓`·이모지는 담지 못합니다. 그런데 이 도구는 그 문자들을 도처에 씁니다 — 일지 항목 헤더가 `⏰`와 `📁`로 만들어져 있으니 **일지 내용을 출력하는 경로는 전부 해당**됩니다
  - 제보된 자리는 `notion_push`의 `print(preview)` 한 줄이었지만, **`src`의 문자열 리터럴 5,741개 중 206개**가 cp949로 인코딩할 수 없는 문자를 담고 있습니다. 그 한 줄을 감싸면 205개가 남습니다
  - 실측(자격증명 없이 도는 명령 11개): **`stats`·`weekly`·`report`·`doctor` 4개가 죽었습니다.** `stats`는 상자 문자 `╔`라서 **출력 첫 글자에서** 죽습니다. 잘 돌고 있는지 알려주는 게 일인 `doctor`가 정작 한글 콘솔에서 못 돌았습니다
  - 진입점에서 한 번 고칩니다. `sys.stdout`/`sys.stderr`를 `errors="replace"`로 바꿉니다
  - **UTF-8을 강제하지 않은 이유**: 레거시 콘솔의 인코딩은 그 터미널이 실제로 그려낼 수 있는 것입니다. UTF-8로 바꾸면 em dash 하나 살리자고 **멀쩡하던 한글을 깨진 글자로** 만듭니다. 반대 거래를 택했습니다 — 한글은 읽히고 대시가 `?`가 됩니다. 전부 원하면 예전처럼 `PYTHONIOENCODING=utf-8`을 주면 됩니다
  - **`hook.py`는 처음부터 이 처리를 하고 있었습니다**(`_configure_stdio`). CLI에만 없었습니다. 이 프로젝트가 반복해서 걸리는 "한 곳은 고쳤고 나머지는 안 고쳤다"가 또 나온 것이고, 그래서 훅 쪽도 테스트로 고정했습니다
  - 회귀 테스트는 `PYTHONIOENCODING=cp949`로 **실제 명령을 별도 프로세스로** 돌립니다. cp949 코덱은 CPython에 기본 포함이라 Linux·macOS CI에서도 같게 돕니다. 수정을 되돌리면 4개가 실패하는 것을 확인했습니다
  - **오늘 돌린 전수 검사가 이걸 못 잡았습니다.** `--help` 스모크는 27개 경로를 다 돌렸지만 출력을 전부 UTF-8 파이프로 받았습니다. 콘솔 인코딩은 검사 항목에 없었습니다
...
- **프로젝트 이름을 저장소 루트로 정규화**: 세션은 한 디렉터리에 머물지 않는다. 가장 큰 transcript 20개 중 **17개**가 여러 `cwd`를 기록하고 한 개는 26종류다 — 하위 폴더로 `cd` 할 때마다 늘어난다. 그런데 프로젝트 이름은 경로의 **마지막 조각**에서 나왔다
  ...
  - `936회 harness → project-d`, `827회 dev → project-a`, `180회 docs → project-b`
  ...
  - 일지와 Notion 양쪽에 같은 규칙을 적용했다. 한쪽만 고치면 로컬은 `project-a`, Notion은 `dev`가 된다
...
- **`agent-diary try`**: 무엇이 기록될지 보려면 기록하게 두는 수밖에 없었다. 뻔한 방법 — `CLAUDE_DIARY_DIR`을 바꿔서 훅을 돌리는 것 — 은 **동작하지 않는다.** `config.json`이 환경변수를 이기도록 설계돼 있어서 일지 경로는 `init`이 쓴 값 그대로고, 시험 실행이 실제 일지에 쌓인다
  ...
  - 인자를 안 주면 **이 디렉터리의 최근 transcript**를 쓴다. 폴더 이름이 아니라 transcript 안의 `cwd`로 찾는다. Claude Code는 경로의 비ASCII 문자를 전부 `-`로 접어서 `C--Users-----Desktop----working-diary` 같은 형태로 만들기 때문에 이름으로는 되돌릴 수 없다
...
- **`변경 통계`가 세션이 한 일이 아니라 커밋 안 한 작업 트리를 재던 문제**: `git diff --stat HEAD`, 즉 **세션 종료 시점의 미커밋 상태**를 재고 있었다. 같은 이름을 쓰는 다른 값이고, 세 방향으로 틀렸다
  ...
  - 생성 파일이 미커밋으로 쌓인 저장소는 그 더미를 **세션마다 다시** 기록한다. 프로젝트 단위로 합치면 작업 트리 하나를 수백 번 센 값이 나온다 — 실측 `project-a` 누적 **-1,547,143줄**
...
- **`task_group` 결손을 push 시점에 드러낸다**: Notion 행을 서로 잇는 유일한 키인데 **62%가 비어 있었다**. 원인은 지침이었다 — "multi-session work용"이라고 적혀 있어서, 이어질 작업인지 아직 모르는 시점에 판단을 미루게 했다. 그건 두 번째 세션이 와야 알 수 있고, 그때는 첫 행이 이미 그룹 없이 기록돼서 **자기 후속과 영원히 연결되지 않는다**
  - 실측: `project-a` 239건 중 222건(93%), `working-diary` 21건 중 15건이 그룹 없음
...
- **`diary-notion push` 끝에 그 프로젝트의 미결 작업 요약 출력**: 이 숫자들은 `diary-notion ops`가 이미 전부 계산하고 있었다. 문제는 아무도 안 본다는 것 — 실측하니 `push`가 2,286회 도는 동안 `notion` 계열은 18회였다. **탐지 수단이 없는 게 아니라 보는 자리에 없었다**
  - 실제 데이터가 왜 필요한지 말해준다: active 476건 중 **7일 이상 방치 441건(93%), 리뷰 대기 218건, next action 없음 125건**. 완료율은 `project-a` 5.9%, `project-d` 3.3%, `solarchive` 0%
