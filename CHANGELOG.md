...
> **한 줄 요약**: 이 도구가 스스로에 대해 조용히 틀린 말을 하고 있던 자리들을
> 찾아 고쳤습니다. 새 명령은 없고, **없던 경고가 몇 군데서 새로 나타납니다.**
>
> 눈에 띄는 것:
>
> - **읽을 수 없는 파일이 "비어 있는 파일"과 같은 답을 내놓던 자리 4곳.** 일지
>   파일이 안 열리면 0세션이었고, 그건 조용한 하루와 구별되지 않았습니다. 이제
>   파일 이름과 함께 말합니다 — 깨끗한 실행에서는 예전처럼 아무 말도 안 합니다
> - **sdist가 자기가 실어 보낸 테스트를 돌릴 수 없었습니다.** 풀어서 `pytest`를
>   돌리면 수집 단계에서 멈췄습니다. 지금은 1,160개가 전부 통과합니다
> - **태그를 밀면 릴리스 노트가 같이 발행됩니다.** 이 릴리스가 그 첫 실행입니다
> - 공개 저장소 소스에 들어가 있던 작성자의 홈 경로를 지웠습니다. **4.10.0까지의
>   sdist·wheel에는 그대로 남아 있습니다** — 회수되지 않습니다
> - 설정은 바뀌지 않았고, 일지 형식도 그대로입니다
